...
#import "JBKHoriBarView.h"

@interface JBKHoriBarView()
...
- (void)drawRect:(CGRect)rect
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        //增加分割线
        CGFloat start = startx + textWidth + marginOfTextAndBar;
        anchorX = start;
        CGRect lineFrame = CGRectMake(start, barMargin_y, self.barMaxWidth, 1);
        ...
        anchorY = CGRectGetMaxY(barFrame) + barMargin;
    }
    
    //添加下方刻度
    int step = self.barMaxWidth / (self.maxValue * 2);//刻度个数，相隔self.maxValue
    UILabel *line = [[UILabel alloc] initWithFrame:CGRectMake(anchorX, anchorY, self.barMaxWidth, 1)];
    line.backgroundColor = [UIColor blackColor];
    [self addSubview:line];
    for (int i = 0; i < step; i ++) {
        CGRect frame = CGRectMake(anchorX + i * self.maxValue * 2, anchorY, 30, 30);
        UILabel *anchor = [[UILabel alloc] initWithFrame:frame];
        anchor.font = [UIFont systemFontOfSize:8.0];
        anchor.text = [NSString stringWithFormat:@"%.0fms",i * self.maxValue * 2];
